...
int main(void)
{
	...
	*/
	...
	double acc_prev_row[3] = { 0 };
	double gyr_prev_row[3] = { 0 };
	double acc[3] = { 0 };
	double gyr[3] = { 0 };
	double z_acc_x[NC] = { 0 };
	double z_acc_y[NC] = { 0 };
	...
	double ccof_lin[ND] = { 0.00006100617862001, 0.00012201235724003, 0.00006100617862001 };
	double dcof_lin[NC] = { 1.0, -1.97778648378264688, 0.97803050849712203 };
	double dt;
	/* END of Luke's code	*/
	...
	while (1) {
		...
		if (new_compass) {
			...
			*/
			...
			/* Perform the JCS conversion in real time and convert values */
			acc[0] = ((double)accel_short[2]) * 2 / 32768; // (g)
			acc[1] = ((double)accel_short[0]) * 2 / 32768; // (g)
			acc[2] = ((double)accel_short[1]) * 2 / 32768; // (g)

			gyr[0] = ((double)gyro[2]) * 2000 / 32768 * PI / 180; // (rad/s)
			gyr[1] = ((double)gyro[0]) * 2000 / 32768 * PI / 180; // (rad/s)
			gyr[2] = ((double)gyro[1]) * 2000 / 32768 * PI / 180; // (rad/s)


			/* Median filter the new data */
...
//			int y = 1;
//			int z = 2;
//			filter_sample(acc, dcof, ccof, x);
//			filter_sample(acc, dcof, ccof, y);
//			filter_sample(acc, dcof, ccof, z);
//			filter_sample(gyr, dcof, ccof, x);
//			filter_sample(gyr, dcof, ccof, y);
//			filter_sample(gyr, dcof, ccof, z);


			sprintf(test_stri, "%li\t%li\t%li\t%li\t%li\t%li\t%llu\r\n", (long)acc[0], (long)acc[1], (long)acc[2], (long)gyr[0], (long)gyr[1], (long)gyr[2], timerValue);
			test_res = f_open(&test_fil, "test.txt", FA_WRITE | FA_OPEN_ALWAYS);
			test_size = (&test_fil)->fsize;
			test_res = f_lseek(&test_fil, test_size);
			...
			acc_norm = pow(acc[0], 2) + pow(acc[1], 2) + pow(acc[2], 2);
			acc_norm = sqrt(acc_norm);

			/* Fill in first 10 rows of normalized accel data */
			if (acc_size <= ACC_LENGTH - 3) {
				acc_filt[acc_size] = acc[0] / acc_norm;
				acc_filt[acc_size + 1] = acc[1] / acc_norm;
				acc_filt[acc_size + 2] = acc[2] / acc_norm;
				acc_size = acc_size + 2;
			}
			else if (acc_size >= 30) {
				int j = 0;
				int index = acc_size - 18;
				...
				acc_filt[ACC_LENGTH - 3] = acc[0] / acc_norm;
				acc_filt[ACC_LENGTH - 2] = acc[1] / acc_norm;
				acc_filt[ACC_LENGTH - 1] = acc[2] / acc_norm;
				dt = ((double)timerValue / 1000) - ((double)acc_prev_time / 1000); // change timer value to seconds
				if(dt > 2.0 || dt < 0){
					for(i = 0; i < 3; i++){
						acc[i] = acc_prev_row[i];
						gyr[i] = gyr_prev_row[i];
					}
				}
				double gyro_vec[4] = { 0,gyr[0], gyr[1], gyr[2] };
				...
				sprintf(euler_stri, "%li\t%li\t%li\t%llu\r\n", (long)euler_angles[0], (long)euler_angles[1], (long)euler_angles[2], timerValue);
				euler_res = f_open(&euler_fil, "euler.txt", FA_WRITE | FA_OPEN_ALWAYS);
				euler_size = (&euler_fil)->fsize;
				euler_res = f_lseek(&euler_fil, euler_size);
			...
			for(i = 0; i < 3; i++){
				acc_prev_row[i] = acc[i];
				gyr_prev_row[i] = gyr[i];
			}
