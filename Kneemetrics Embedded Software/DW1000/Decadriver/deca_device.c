...
#if ENABLE_PRINTF
    	sleep(2);
    	printf("\n\r dwt_initialise readdevid error");
#endif
        return DWT_ERROR ;
    }
#if ENABLE_PRINTF
 	sleep(2);
    printf("\n\r dwt_initialise readdevid PASSED");
#endif
...
#if ENABLE_PRINTF
 	sleep(2);
    printf("\n\r dwt_initialise PART ID %08x", dw1000local.partID);
 	sleep(2);
 	printf("\n\r dwt_initialise LOT ID %08x", dw1000local.lotID);
#endif
...

		{
				...
				else //double buffer
	        {
	            ...
	            uint8 hsrb = 0x01 ;
					...
					sprintf(stt, "%x",buff);//prepare string to be sent through uart

					    			           			//TX_ExtUART(stri);//send result to UART
					    			           			USART_puts(USART6,stt);
