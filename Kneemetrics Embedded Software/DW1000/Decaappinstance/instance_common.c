...
int instance_init(void)
{
    ...
    printf("\n\r Initializeing SWT");
#endif
	//we can enable any configuration loading from OTP/ROM on initialisation
...
#if ENABLE_PRINTF
    Sleep(100);
    printf("\n\r instance Clear events PASSED");
#endif
...
int instance_run(void)
{
   ...
   // printf("\r\n In Instance Run");
    ...
    //printf("\r\n Event Peeked Message %d", message);
   ...
   // printf("\r\n Instance Done");
    ...
    if((instance_data[instance].instancetimer_en == 1) && (instance_data[instance].stoptimer == 0))
    {
        if(instance_data[instance].instancetimer < portGetTickCount())
        {
			...
			instance_putevent(dw_event);
#if ENABLE_PRINTF
			printf("\r\n Instance RUN timeout DWT_SIG_RX_TIMEOUT");
#endif
        }
    }

  //  printf("\r\n Return from Instance Run");
    return 0 ;
}
