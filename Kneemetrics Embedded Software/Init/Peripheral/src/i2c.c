...
#define FUEL_I2C_SCL_GPIO_PORT         GPIOH
#define FUEL_I2C_SCL_GPIO_CLK          RCC_AHB1Periph_GPIOH
#define FUEL_I2C_SCL_GPIO_PIN          GPIO_Pin_7
#define FUEL_I2C_SCL_GPIO_PINSOURCE    GPIO_PinSource7

#define FUEL_I2C_SDA_GPIO_PORT         GPIOH
#define FUEL_I2C_SDA_GPIO_CLK          RCC_AHB1Periph_GPIOH
#define FUEL_I2C_SDA_GPIO_PIN          GPIO_Pin_8
#define FUEL_I2C_SDA_GPIO_PINSOURCE    GPIO_PinSource8

#define FUEL_I2C_RCC_CLK               RCC_APB1Periph_I2C3
#define FUEL_I2C_AF                    GPIO_AF_I2C3
...
#define WAIT_FOR_FLAG_FUEL(flag, value, timeout, errorcode)  I2CTimeout = timeout;\
          while(I2C_GetFlagStatus(FUEL_I2C, flag) != value) {\
            if((I2CTimeout--) == 0) return I2Cx_TIMEOUTFUEL_UserCallback(errorcode); \
          }\
  
#define CLEAR_ADDR_BIT      I2C_ReadRegister(SENSORS_I2C, I2C_Register_SR1);\
                            I2C_ReadRegister(SENSORS_I2C, I2C_Register_SR2);\

#define CLEAR_ADDR_BIT_FUEL     I2C_ReadRegister(FUEL_I2C, I2C_Register_SR1);\
                            	I2C_ReadRegister(FUEL_I2C, I2C_Register_SR2);\
...
unsigned long ST_Fuel_I2C_WriteRegister(unsigned char Address, unsigned char RegisterAddr, unsigned short RegisterLen, const unsigned char *RegisterValue);
unsigned long ST_Fuel_I2C_ReadRegister(unsigned char Address, unsigned char RegisterAddr, unsigned short RegisterLen, unsigned char *RegisterValue);
/*******************************  Function ************************************/
...
void I2cMasterFuel_Init(void)
{
  GPIO_InitTypeDef GPIO_InitStructure;
  I2C_InitTypeDef I2C_InitStructure;

    /* Enable I2Cx clock */
  RCC_APB1PeriphClockCmd(FUEL_I2C_RCC_CLK, ENABLE);

  /* Enable I2C GPIO clock */
  RCC_AHB1PeriphClockCmd(FUEL_I2C_SCL_GPIO_CLK | FUEL_I2C_SDA_GPIO_CLK, ENABLE);

  /* Configure I2Cx pin: SCL ----------------------------------------*/
  GPIO_InitStructure.GPIO_Pin =  FUEL_I2C_SCL_GPIO_PIN;
  GPIO_InitStructure.GPIO_Mode = GPIO_Mode_AF;
  GPIO_InitStructure.GPIO_Speed = GPIO_Speed_100MHz;
  GPIO_InitStructure.GPIO_OType = GPIO_OType_OD;
  GPIO_InitStructure.GPIO_PuPd  = GPIO_PuPd_NOPULL;
  ...
  /* Connect pins to Periph */
  GPIO_PinAFConfig(FUEL_I2C_SCL_GPIO_PORT, FUEL_I2C_SCL_GPIO_PINSOURCE, FUEL_I2C_AF);
  GPIO_Init(FUEL_I2C_SCL_GPIO_PORT, &GPIO_InitStructure);

  /* Configure I2Cx pin: SDA ----------------------------------------*/
  GPIO_InitStructure.GPIO_Pin = FUEL_I2C_SDA_GPIO_PIN;

  /* Connect pins to Periph */
  GPIO_PinAFConfig(FUEL_I2C_SDA_GPIO_PORT, FUEL_I2C_SDA_GPIO_PINSOURCE, FUEL_I2C_AF);
  GPIO_Init(FUEL_I2C_SDA_GPIO_PORT, &GPIO_InitStructure);
  ...
  /* Enable the I2C peripheral */
  I2C_Cmd(FUEL_I2C, ENABLE);

  /* Initialize the I2C peripheral */
  I2C_Init(FUEL_I2C, &I2C_InitStructure);
...
/**
  ...
  * @retval None.
  */
static uint32_t I2Cx_TIMEOUTFUEL_UserCallback(char value)
{
  ...
  I2C_GenerateSTOP(FUEL_I2C, ENABLE);
  I2C_SoftwareResetCmd(FUEL_I2C, ENABLE);
  I2C_SoftwareResetCmd(FUEL_I2C, DISABLE);
...
int Fuel_I2C_WriteRegister(unsigned char slave_addr,
                                        unsigned char reg_addr,
                                        unsigned short len,
                                        const unsigned char *data_ptr)
{
...
tryWriteAgain:
  ret = 0;
  ret = ST_Fuel_I2C_WriteRegister( slave_addr, reg_addr, len, data_ptr);
...
int Fuel_I2C_ReadRegister(unsigned char slave_addr,
                                       unsigned char reg_addr,
                                       unsigned short len,
                                       unsigned char *data_ptr)
{
...
tryReadAgain:
  ret = 0;
  ret = ST_Fuel_I2C_ReadRegister( slave_addr, reg_addr, len, data_ptr);
...
//  RegisterValue = RegisterValue + (RegisterLen - 1);
  ...
  /* Wait for the busy flag to be cleared */
  WAIT_FOR_FLAG_FUEL (I2C_FLAG_BUSY, RESET, I2Cx_LONG_TIMEOUT, 1);

  /* Start the config sequence */
  I2C_GenerateSTART(FUEL_I2C, ENABLE);

  /* Wait for the start bit to be set */
  WAIT_FOR_FLAG_FUEL (I2C_FLAG_SB, SET, I2Cx_FLAG_TIMEOUT, 2);

  /* Transmit the slave address and enable writing operation */
  I2C_Send7bitAddress(FUEL_I2C, (Address<<1), I2C_Direction_Transmitter);

  /* Wait for address bit to be set */
  WAIT_FOR_FLAG_FUEL (I2C_FLAG_ADDR, SET, I2Cx_FLAG_TIMEOUT, 3);

  /* clear the ADDR interrupt bit  - this is done by reading SR1 and SR2*/
  CLEAR_ADDR_BIT_FUEL;

  /* Wait for address bit to be set */
  WAIT_FOR_FLAG_FUEL (I2C_FLAG_TXE, SET, I2Cx_FLAG_TIMEOUT, 4);

  /* Transmit the first address for write operation */
  I2C_SendData(FUEL_I2C, RegisterAddr);
...
//  }
  ...
  {
    /* Wait for address bit to be set */
    WAIT_FOR_FLAG_FUEL (I2C_FLAG_TXE, SET, I2Cx_FLAG_TIMEOUT, 5);

    /* Prepare the register value to be sent */
    I2C_SendData(FUEL_I2C, RegisterValue[i]);
  }

  /* Wait for address bit to be set */
  WAIT_FOR_FLAG_FUEL (I2C_FLAG_BTF, SET, I2Cx_FLAG_TIMEOUT, 6);
...
unsigned long ST_Fuel_I2C_ReadRegister(unsigned char Address, unsigned char RegisterAddr, unsigned short RegisterLen, unsigned char *RegisterValue)
{
  ...
  /* Wait for the busy flag to be cleared */
  WAIT_FOR_FLAG_FUEL (I2C_FLAG_BUSY, RESET, I2Cx_LONG_TIMEOUT, 7);

  /* Start the config sequence */
  I2C_GenerateSTART(FUEL_I2C, ENABLE);

  /* Wait for the start bit to be set */
  WAIT_FOR_FLAG_FUEL (I2C_FLAG_SB, SET, I2Cx_FLAG_TIMEOUT, 8);

  /* Transmit the slave address and enable writing operation */
  I2C_Send7bitAddress(FUEL_I2C, (Address<<1), I2C_Direction_Transmitter);

  /* Wait for the start bit to be set */
  WAIT_FOR_FLAG_FUEL (I2C_FLAG_ADDR, SET, I2Cx_FLAG_TIMEOUT, 9);

  /* clear the ADDR interrupt bit  - this is done by reading SR1 and SR2*/
  CLEAR_ADDR_BIT_FUEL;

  /* Wait for address bit to be set */
  WAIT_FOR_FLAG_FUEL (I2C_FLAG_TXE, SET, I2Cx_FLAG_TIMEOUT, 10);

  /* Transmit the register address to be read */
  I2C_SendData(FUEL_I2C, RegisterAddr);

  /* Wait for address bit to be set */
  WAIT_FOR_FLAG_FUEL (I2C_FLAG_TXE, SET, I2Cx_FLAG_TIMEOUT, 11);

  /*!< Send START condition a second time */
  I2C_GenerateSTART(FUEL_I2C, ENABLE);

  /* Wait for the start bit to be set */
  WAIT_FOR_FLAG_FUEL (I2C_FLAG_SB, SET, I2Cx_FLAG_TIMEOUT, 12);

  /*!< Send address for read */
  I2C_Send7bitAddress(FUEL_I2C, (Address<<1), I2C_Direction_Receiver);

  /* Wait for the start bit to be set */
  WAIT_FOR_FLAG_FUEL (I2C_FLAG_ADDR, SET, I2Cx_FLAG_TIMEOUT, 13);

  if (RegisterLen == 1)
  {
    /*!< Disable Acknowledgment */
    I2C_AcknowledgeConfig(FUEL_I2C, DISABLE);
    ...
    /* Wait for the RXNE bit to be set */
    WAIT_FOR_FLAG_FUEL (I2C_FLAG_RXNE, SET, I2Cx_FLAG_TIMEOUT, 14);

    RegisterValue[0] = I2C_ReceiveData(FUEL_I2C);
  }
  else if( RegisterLen == 2)
  {
     /*!< Disable Acknowledgment */
   ...
   /* Set POS bit */
   FUEL_I2C->CR1 |= I2C_CR1_POS;

   /* clear the ADDR interrupt bit  - this is done by reading SR1 and SR2*/
   CLEAR_ADDR_BIT_FUEL;

   /* Wait for the buffer full bit to be set */
   WAIT_FOR_FLAG_FUEL (I2C_FLAG_BTF, SET, I2Cx_FLAG_TIMEOUT, 15);
   ...
   /* Read 2 bytes */
   RegisterValue[0] = I2C_ReceiveData(FUEL_I2C);
   RegisterValue[1] = I2C_ReceiveData(FUEL_I2C);
  }
  else if( RegisterLen == 3)
  {
    CLEAR_ADDR_BIT_FUEL;
    ...
    /* Wait for the buffer full bit to be set */
    WAIT_FOR_FLAG_FUEL (I2C_FLAG_BTF, SET, I2Cx_FLAG_TIMEOUT, 16);
    /*!< Disable Acknowledgment */
    I2C_AcknowledgeConfig(FUEL_I2C, DISABLE);
    /* Read 1 bytes */
    RegisterValue[0] = I2C_ReceiveData(FUEL_I2C);
    ...
    RegisterValue[1] = I2C_ReceiveData(FUEL_I2C);
    /* Wait for the buffer full bit to be set */
    WAIT_FOR_FLAG_FUEL (I2C_FLAG_RXNE, SET, I2Cx_FLAG_TIMEOUT, 17);
    /* Read 1 bytes */
    RegisterValue[2] = I2C_ReceiveData(FUEL_I2C);
  }
  else /* more than 2 bytes */
  {
    /* clear the ADDR interrupt bit  - this is done by reading SR1 and SR2*/
    ...
    for(i=0; i<(RegisterLen); i++)
    {
      if(i==(RegisterLen-3))
      {
        ...
        /* Read 1 bytes */
        RegisterValue[i++] = I2C_ReceiveData(FUEL_I2C);
      ...
      /* Wait for the RXNE bit to be set */
      WAIT_FOR_FLAG_FUEL (I2C_FLAG_RXNE, SET, I2Cx_FLAG_TIMEOUT, 18);
      RegisterValue[i] = I2C_ReceiveData(FUEL_I2C);
    }
  }
...
endReadLoop:
  /* Clear BTF flag */
  I2C_ClearFlag(FUEL_I2C, I2C_FLAG_BTF);
  /* Wait for the busy flag to be cleared */
  WAIT_FOR_FLAG_FUEL (I2C_FLAG_BUSY, RESET, I2Cx_LONG_TIMEOUT, 19);
  /*!< Re-Enable Acknowledgment to be ready for another reception */
  I2C_AcknowledgeConfig(FUEL_I2C, ENABLE);
  //Disable POS -- TODO
  FUEL_I2C->CR1 &= ~I2C_CR1_POS;
