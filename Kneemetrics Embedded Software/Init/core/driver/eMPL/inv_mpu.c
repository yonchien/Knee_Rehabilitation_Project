...
#ifdef AK89xx_SECONDARY
    setup_compass();
    if (mpu_set_compass_sample_rate(500))//10
        return -1;
#else
...
restore:
    ...
    st.chip_cfg.fifo_enable = 0xFF;
    st.chip_cfg.clk_src = INV_CLK_PLL;
    mpu_set_gyro_fsr(2000);//(gyro_fsr);
    mpu_set_accel_fsr(2);//(accel_fsr);
    mpu_set_lpf(lpf);
    mpu_set_sample_rate(sample_rate);
    mpu_set_sensors(sensors_on);
    mpu_configure_fifo(fifo_sensors);
...
lp_int_restore:
    ...
    mpu_set_gyro_fsr(2000);//(st.chip_cfg.cache.gyro_fsr);
    mpu_set_accel_fsr(2);//(st.chip_cfg.cache.accel_fsr);
...
/* mar, additional helper functions */
/* returns current FIFO count or error code -1*/
short mpu_get_fifo_count(void)
{
	unsigned char tmp[2];
	short fifo_count;
